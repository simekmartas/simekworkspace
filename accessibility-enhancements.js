...
class AccessibilityEnhancements {
  ...
  setupAccessibilityToolbar() {
    ...
    toolbar.setAttribute('aria-label', 'Nástroje přístupnosti');
    
    toolbar.innerHTML = `
      <button class="a11y-btn" data-action="increase-text" aria-label="Zvětšit text">A+</button>
      <button class="a11y-btn" data-action="decrease-text" aria-label="Zmenšit text">A-</button>
      <button class="a11y-btn" data-action="high-contrast" aria-label="Vysoký kontrast">⚫⚪</button>
      <button class="a11y-btn" data-action="focus-mode" aria-label="Režim focusu">🎯</button>
      <button class="a11y-btn" data-action="reading-guide" aria-label="Čtecí vodítko">📏</button>
    `;
    ...
    this.addStyles('accessibility-toolbar-styles', toolbarStyles);
    
    document.body.appendChild(toolbar);
    
    // Event listenery pro toolbar
    toolbar.addEventListener('click', (e) => {
      if (e.target.matches('.a11y-btn')) {
        this.handleToolbarAction(e.target.dataset.action, e.target);
      }
    });
  }
