...
class UserProfileDataLoader {
    ...
    async updateUserRanking(rankingElement, isMonthly) {
        ...
        try {
            // Nastavit "Načítám..." během načítání
            rankingElement.textContent = 'Načítám...';
            
            // Načíst žebříček přímo - stejná logika jako v leaderboards-data-loader.js
            const leaderboardData = await this.loadLeaderboardForRanking();
            
            if (leaderboardData && leaderboardData.length > 0) {
                // Najít pozici uživatele v žebříčku
                const userSellerIdStr = String(this.userSellerId);
                const userIndex = leaderboardData.findIndex(seller => 
                ...
                console.log('⚠️ Žebříček je prázdný');
                rankingElement.textContent = '?';
            }
    ...
    // NOVÁ metoda pro načtení žebříčku pro ranking
    async loadLeaderboardForRanking() {
        console.log('=== NAČÍTÁNÍ ŽEBŘÍČKU PRO POZICI ===');
        
        try {
            // 1. Načíst uživatele ze serveru
            const usersData = await this.loadUsersFromServerForRanking();
            
            // 2. Načíst měsíční data z Google Sheets (žebříček používá vždy měsíční data)
            const salesData = await this.loadSalesDataForRanking();
            
            // 3. Spočítat žebříček
            const leaderboard = this.calculateLeaderboardForRanking(usersData, salesData);
            
            console.log(`🏆 Žebříček pro pozici načten: ${leaderboard.length} prodejců`);
            
            return leaderboard;
            
        } catch (error) {
            console.error('❌ Chyba při načítání žebříčku pro pozici:', error);
            return [];
        }
    }

    // Načíst uživatele ze serveru pro ranking
    async loadUsersFromServerForRanking() {
        try {
            console.log('🌐 Načítám uživatele ze serveru pro ranking...');
            const response = await fetch('/api/users-github', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
            ...
            if (data.success && Array.isArray(data.users)) {
                ...
                console.log(`👥 Načteno ${sellers.length} prodejců pro ranking`);
            ...
            console.error('❌ Chyba při načítání uživatelů pro ranking:', error);
            throw error;
        }
    }

    // Načíst prodejní data pro ranking
    async loadSalesDataForRanking() {
        return new Promise((resolve, reject) => {
            const timestamp = Date.now();
            const callbackName = `rankingCallback_${timestamp}_${Math.random().toString(36).substr(2, 9)}`;
            
            // Vytvoř globální callback funkci
            window[callbackName] = (data) => {
                try {
                    console.log('JSONP callback pro ranking zavolán');
                    
                    if (data && data.data && Array.isArray(data.data)) {
                        const csvData = this.convertJsonToCsv(data.data);
                        resolve(csvData);
                    } else {
                        reject(new Error('Neplatná struktura dat pro ranking'));
                    }
                    ...
                    console.error('Chyba v JSONP callback pro ranking:', error);
                    reject(error);
                }
            };
            
            // Vytvoř script tag - použijeme měsíční data (stejné jako žebříček)
            const script = document.createElement('script');
            ...
            script.onerror = () => {
                ...
                reject(new Error('JSONP request failed pro ranking'));
            };
            ...
            setTimeout(() => {
                if (window[callbackName]) {
                    ...
                    reject(new Error('JSONP request timeout pro ranking'));
                }
            }, 10000);
    ...
    // Spočítat žebříček pro ranking
    calculateLeaderboardForRanking(usersData, csvData) {
        console.log('=== VÝPOČET ŽEBŘÍČKU PRO POZICI ===');
        
        const lines = csvData.split('\n').filter(line => String(line || '').trim());
        
        if (lines.length === 0) {
            throw new Error('Žádná data v tabulce pro ranking');
        }
        ...
        if (idProdejceIndex === -1) {
            throw new Error('Sloupec "id_prodejce" nenalezen pro ranking');
        }
        ...
        Object.keys(usersData).forEach(sellerId => {
            ...
            if (userRows.length > 0) {
                // Spočítat body - použijeme stejnou logiku jako v updateUserPoints
                const points = this.calculateUserPointsForRanking(userRows, headers);
        ...
        console.log(`🏆 Žebříček pro pozici spočítán: ${leaderboard.length} prodejců`);
        leaderboard.forEach((seller, index) => {
            console.log(`${index + 1}. ${seller.name}: ${seller.points} bodů`);
        });
        
        return leaderboard;
    }

    // Spočítat body prodejce pro ranking (stejná logika jako updateUserPoints)
    calculateUserPointsForRanking(rows, headers) {
