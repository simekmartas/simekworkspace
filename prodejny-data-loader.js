...
class ProdejnyDataLoader {
    ...
    // Metoda pro zobrazení historických dat
    displayHistoricalData(data, date) {
        console.log('=== ZOBRAZUJEM HISTORICKÁ DATA ===');
        ...
        // Seřadit podle sloupce polozky_nad_100 (nebo podobného)
        const polozkyIndex = headers.findIndex(h => h.toLowerCase().includes('polozky'));
        if (polozkyIndex >= 0) {
            const sortedRows = this.sortRowsByColumn(rows, polozkyIndex);
            this.displayTableWithDate(headers, sortedRows, this.isMonthly, date);
        } else {
            this.displayTableWithDate(headers, rows, this.isMonthly, date);
        }
        
        // Zastavit auto-refresh pro historická data
        this.stopAutoRefresh();
    }

    // Upravená displayTable pro zobrazení data
    displayTableWithDate(headers, rows, isMonthly, historicalDate = null) {
        // Najít krále
        const { polozkyKing, sluzbyKing } = this.findKings(rows);
        ...
        const dateInfo = historicalDate ? 
            `<span style="color: #ff0040; margin-left: 10px;">[HISTORICKÁ DATA: ${new Date(historicalDate).toLocaleDateString('cs-CZ')}]</span>` : 
            '';

        this.container.innerHTML = `
            <div class="retro-data-container">
                <div class="retro-data-header">
                    <span class="retro-terminal-prompt">&gt; prodejny_${isMonthly ? 'monthly' : 'current'}.csv_${dateInfo}</span>
                    <div class="retro-window-controls">
                        <span class="control-dot red"></span>
                        <span class="control-dot yellow"></span>
                        <span class="control-dot green"></span>
                ...
                <div class="retro-data-footer">
                    <div class="retro-status-left">
                        <span class="retro-status-text">// PRODEJNY DATA ${historicalDate ? 'ARCHIVE' : 'STREAM'} ACTIVE</span>
                        <span class="retro-blink">●</span>
                    </div>
                    <div class="retro-status-right">
                        <span class="retro-data-flow">▓▒░</span>
                        <span class="retro-small-text">SYNC: 100% | ${historicalDate ? `DATA Z: ${new Date(historicalDate).toLocaleDateString('cs-CZ')}` : `AKTUALIZOVÁNO: ${new Date().toLocaleTimeString('cs-CZ')}`}</span>
                    </div>
                </div>
            </div>
        `;
